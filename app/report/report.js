(function (app) {
  ...
  angular.module('baltcoGo.report', ['ngRoute'])
    ...
    .controller('ReportCtrl', ['$http', '$scope', '$timeout', 'mapServiceComposite', 'reportService', 'urls',
      function ($http, $scope, $timeout, mapServiceComposite, reportService, urlsProvider) {
        ...
        function getFirstSuggestion() {
          var pacItem = angular.element(document).find('.pac-item').first(),
        ...
        function autocompleteHandler(event) {
          ...
          if (keycode === 40) {
            ...
            angular.element(document).find('.autocomplete-results button').first().focus();
            return;
          }
        ...
        function autocompleteResultButtonKeyboardNavigationHandler(event) {
          var keycode = event.which || event.keyCode;
          var $target = angular.element(document).find(event.target);

          if (!$target.is('.autocomplete-results button')) {
            return;
          }

          if (keycode === 27 && angular.element(document).find('.autocomplete-results').is(':visible')) {
            self.autocompleteResults = [];
            $scope.$apply();
          }
        ...
        function displayAddressError() {
          angular.element(document).find('#map').closest('cs-form-control').addClass('error');
        ...
        function mapClickHandler(event) {
          var $wrapper = angular.element(document).find('#map').closest('cs-form-control'),
